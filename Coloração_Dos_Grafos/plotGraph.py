...
class Graph():
    ...
    def _createGraph(self):
        ...
        for i, vertice in enumerate(self.vertices):
            if isinstance(vertice, list) and len(vertice) == 2:
                Graph.add_node(i + 1, nome=vertice[0], legenda=vertice[1])
            else:
                Graph.add_node(i + 1, nome=vertice[0])
    ...
    def printColor(self, index_color, colors, pause=False):
        node_color = ["lightblue" for node in self.graph.nodes]
        for i, color in enumerate(index_color):
            node_color[i] = colors[color]
